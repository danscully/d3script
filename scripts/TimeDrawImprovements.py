...
def _renderToDisplayListR30(view, layer, isSelected):
    view.LAYER_RENDER_HEIGHT = 22 * d3gui.dpiScale.x
    displayList = layer.subdl
    displayList.clear()
    whiteMix = colours('layer_whitemix')
    ...
    expandIconOffset = view.UI_ELEMENT_PADDING
    textOffset = view.UI_ELEMENT_PADDING
    parentOffset = view.barWidget.parent.absPos.x
    ...
    for key in keys:
        displayList.quad(view.endMaterial, Rect(Vec2(bw.tToX(key.localT - layer.start) - view.END_BITMAP_SIZE.x/2 -1,(6 * d3gui.dpiScale.x)), view.END_BITMAP_SIZE*1.3), keyCol, Rect(0, 0, 1, 1))
        displayList.quad(view.endMaterial, Rect(Vec2(bw.tToX(key.localT - layer.start)  - view.END_BITMAP_SIZE.x/2+2,(6 * d3gui.dpiScale.x)), view.END_BITMAP_SIZE*1.3), keyCol, Rect(0, 0, -1, 1))

    if isSelected:
        displayList.quad(view.endMaterial, Rect(Vec2(), view.END_BITMAP_SIZE), iconCol, Rect(0, 0, 1, 1))
        displayList.quad(view.endMaterial, Rect(Vec2(layer.size.x - view.END_BITMAP_SIZE.x, 0), view.END_BITMAP_SIZE), iconCol, Rect(1, 0, -1, 1))
        textOffset += view.END_BITMAP_SIZE.x
        expandIconOffset += view.END_BITMAP_SIZE.x
    if layer.isGroup:
        leftMostPos = parentOffset + expandIconOffset
        rightMostPos = view.barWidget.absPos.x + layer.pos.x + layer.size.x - view.EXPAND_BITMAP_SIZE - view.UI_ELEMENT_PADDING
        if leftMostPos < rightMostPos:
            if layer.isSmart:
                material = view.smartGroupMaterial
            else:
                material = view.contractMaterial if layer.isExpanded else view.expandMaterial
            displayList.quadBounded(material, Rect(Vec2(expandIconOffset, 5), Vec2(view.EXPAND_BITMAP_SIZE, view.EXPAND_BITMAP_SIZE)), iconCol, Rect(0, 0, 1, 1), leftMostPos)
            textOffset += view.EXPAND_BITMAP_SIZE + view.UI_ELEMENT_PADDING
    name = layer.name + layer.indicator + (' [LOCKED]' if layer.locked else '')
    extent = layer.size.x - textOffset - view.UI_ELEMENT_PADDING
    displayList.printBounded(d3gui.font, name, Vec2(textOffset, 1), max(0, extent), parentOffset + textOffset, 1, colours('layer_name'))
    ...
    if controlPatch:
        ...
        ext = d3gui.font.extent(name + '  ')
        labelOffset = textOffset + ext.x
        displayList.printBounded(d3gui.font, patchText, Vec2(labelOffset, 1), max(0, extent - ext.x), parentOffset + labelOffset, 1, colours('layer_name') * 0.75)
    return
